...
from revng.pypeline.utils.registry import get_singleton, get_registry
...
class Response:
    ...
    body: Any
    """
    The respone body of the request.
    """
...
class Daemon:
    ...
    def get_epoch(self, request) -> Response:
        return Response(
            code=200,
            body={
                "epoch": self.lock.get_epoch(request["project_id"])
            }
        )
        ...
        return Response(
            ...
            body={
                ...
                "containers": [
                    ...
                    for name, container in get_registry(Container).items() # type: ignore [type-abstract]
                ],
                "artifacts": [
                    {
                        "name": artifact.name,
                        ...
                        # An artifact is cachable if it's backed by a savepoint.
                        "cachable": isinstance(artifact.node.task, SavePoint),
                    }
        ...
        for container_name, objects in containers.items():
            for decl in self.pipeline.declarations:
                ...
                return Response(code=400, body={
                    ...
                    "available_containers": list(sorted(decl.name for decl in self.pipeline.declarations)),
                })
            ...
            websocket=[
                {
                    ...
                    "diff": diff,
                    "invalidated": [
                        # WIP List of invalidated cachable artifacts (and the objects inside it)
                    ]
                }
            ]
        )
