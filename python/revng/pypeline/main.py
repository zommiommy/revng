...
def autocomplete(ctx, shell):
    ...
    # figure out if we are revng2 or pipe
    prog_name = root_cmd.name
...
@click.option(
    ...
    type=EagerParsedPath(
        name="pipebox",
        parser=lambda path: import_pipebox(
            path,
            any(x.endswith("_COMPLETE") for x in os.environ),
        ),
    ),
    help=("Path to the pipebox file. Defaults to the `PIPEBOX` environment " "variable if set."),
